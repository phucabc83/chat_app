...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  AuthBloc(this.loginUseCase, this.signUpUseCase, this.authGoogleRepository,
      this.authFacebookRepository)
      : super(AuthInitial()) {
    ...
    on<AuthGoogleSignInEvent>((event, emit) async {
      ...
      try {
        ...
        if (result.user == null) {
          emit(
              AuthFailure(
                  'Đăng nhập Google thất bại: Không nhận được thông tin người dùng.')
          );
          return;
        }

        final user = await authGoogleRepository.getUserDB(
            email: result.user!.email!,
            name: result.user!.displayName ?? result.user!.email!,
            avatarUrl: result.user!.photoURL ??
                'https://placehold.co/100x100/png?text=${result.user!
